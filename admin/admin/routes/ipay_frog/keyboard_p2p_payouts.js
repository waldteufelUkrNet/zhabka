...
router.post('/list', async function (req, res, next) {
    ...
    let list = await api.KeyboardP2pPayoutBody.findAll(query);
...
router.post('/count', async function (req, res, next) {
    ...
    query.order = [['id', 'DESC']];
    let count = await api.KeyboardP2pPayoutBody.count(query);
...
router.get('/info/:id', async function (req, res, next) {
    if(!req.input_user || !req.input_user.user)
        ...
        let item = await api.KeyboardP2pPayoutBody.findOne({
