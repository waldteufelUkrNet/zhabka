...
import {KeyboardP2pPayoutService, KeyboardPaymentDetailsService} from '../../shared';
import {NgbModal} from '@ng-bootstrap/ng-bootstrap';
import * as moment from 'moment'

@Component({
  selector: 'app-p2p-payout',
  templateUrl: './p2p_payout.component.html',
  styleUrls: ['./p2p_payout.component.scss'],
  providers: [KeyboardP2pPayoutService, KeyboardPaymentDetailsService]
})
export class P2pPayoutComponent implements OnInit {
  search = '';
  searchTimeout;
  list;
  countList;
  ...
  showShows(item){
    this.paymentDetailsService.getShowsList({payment_id:item.id, payment_type: 'p2p_payout'})
    .then((mList: any) => {
      this.itemShows = mList;
    })
  }
  showSender(item){
    this.paymentDetailsService.getSender({payment_id:item.id, payment_type: 'p2p_payout'})
    .then((mList: any) => {
